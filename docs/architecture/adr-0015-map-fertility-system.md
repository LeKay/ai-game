...
## Addendum (2026-06-22): Ore & Gem Deposits

The clay deposit mechanic is generalized from clay-only to **N mineable deposits**. Six new
fertilities join the pool: common **iron / copper / tin** and rare **silver / gold / gemstones**.
See `design/quick-specs/ore-gem-deposits-2026-06-22.md`.

- **Weighted roll:** `WorldGrid.FERTILITY_WEIGHTS` (common 12, precious 1) replaces the uniform
  shuffle; a shared static `WorldGrid.roll_fertility(seed, count)` is used by both `WorldGrid` and
  `OverworldSystem` (single source of truth). Precious deposits appear ~1/12 as often.
- **Generalized deposit layer:** `_hidden_resources` now holds any deposit id. `DEPOSIT_TILE_TYPE`
  maps each deposit fertility → pit `TileType`; `DEPOSIT_COUNTS` sets scatter counts (common 6,
  precious 1). `_populate_hidden_clay` → `_populate_hidden_deposits` (one deposit per tile),
  `reveal_hidden_clay` → `reveal_hidden_deposit` (returns the revealed id).
- **Search is deposit-agnostic:** `survey_tile` reveals whatever sits on-tile, else reports the
  nearest hidden deposit of any kind + its name (`find_nearest_any_hidden`). Result fields renamed
  `clay_*` → `deposit_*` (+ `deposit_id`).
- **Mining:** six `MINE_<ORE>` manual actions (clay parity), all gated by the **Prospecting** node
  alongside `MINE_CLAY`.
- **Enum stability:** `IRON…GEMSTONE` appended after `COAST`; saved terrain unchanged.
- **Scope:** deposit + manual mine only — per-ore extraction buildings deferred (as clay's was).

## Addendum (2026-06-23): Second Fertility Wave — Crops, Groves, Sand, Marble, Amber, Pearls

Nine new fertilities join the pool, each reusing one of three existing placement/render patterns
(no new rendering layer). Restricted by **biome** at roll time. No buildings yet — all are
hand-gatherable map resources.

| Fertility | Render pattern | Tile mechanic | Biome bias |
|-----------|----------------|---------------|------------|
| flax / hops / grapes | opaque field crop (wheat-like) on `EMPTY` | Harvest action | ×2 on plains |
| olives / bees(honey) | transparent overlay over a `GRASS` base | Harvest action | ×2 on plains |
| marble | transparent overlay ("like stone") on `STONE`/`EMPTY` | Mine action | ×3 on mountain |
| sand | opaque beach tile on land bordering `WATER`/`COAST` | Gather action | coast only |
| amber | hidden deposit (clay-like), Search-revealed → `AMBER` overlay | Mine, Prospecting-gated | coast/forest only |
| pearl | visible overlay on ocean `COAST` (impassable like water) | none (decorative) | coast only |

- **Biome-aware roll:** `WorldGrid.roll_fertility(seed, count, biome)` gains an optional `biome`
  arg (`BIOME_ANY` default = pre-addendum behaviour). `_biome_fertility_weight` applies hard zeros
  (pearl/sand/amber outside their biomes never roll) and soft multipliers (crops on plains, marble
  on mountain). `OverworldSystem` maps each tile's `Biome` → `BIOME_*` via `_fertility_biome_class`.
- **Placement:** new deterministic `generate()` passes — `_populate_field_crops`,
  `_populate_grass_overlays`, `_populate_marble`, `_populate_sand_beaches`, `_populate_pearls` —
  each seeded by a distinct large offset. Amber rides the existing `_populate_hidden_deposits`
  machinery via `DEPOSIT_TILE_TYPE`/`DEPOSIT_COUNTS`.
- **Rendering:** `terrain_renderer.gd` gains 9 PNG-variant lists + 9 fallback colours (color
  placeholders until art is generated) and a `_TERRAIN_BASE_TYPE` map so `sync()` draws the correct
  background under transparent overlays (`GRASS` under olive/bees, `COAST` under pearl); everything
  else composites over sand, matching how tree/stone overlays already work.
- **Actions:** `HARVEST_FLAX/HOPS/GRAPES/OLIVES/HONEY`, `GATHER_SAND`, `MINE_MARBLE` (ungated,
  available by default) and `MINE_AMBER` (added to the Prospecting node). `_terrain_to_action`
  maps each new tile type; pearls have no interaction.
- **Passability:** `PEARL` is impassable (`_blocks_occupation` + `INF` movement cost); the other
  eight are walkable resource tiles (cost 4.0), matching clay/ore.
- **Enum stability:** `FLAX…PEARL` appended after `GEMSTONE` (ordinals 16-24); saved terrain
  unchanged.
- **Scope / deferred:** no consumer/processing buildings; pearls are visible-at-spawn (not a Search
  resource). Tile/icon art is generated by dedicated PixelLab scripts under
  `assets/art/ai-prompts/` (run separately); code falls back to colours/glyphs until art lands.
