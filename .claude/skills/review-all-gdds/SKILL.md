...
1. **Cross-GDD Consistency** â€” contradictions, stale references, and ownership
   conflicts between documents
2. **Game Design Holism** â€” issues that only emerge when you see all systems
...
### Phase 1a â€” L0: Summary Scan (fast, low tokens)
...
Found [N] GDDs. Summaries:
  â€¢ combat.md â€” [summary text]
  â€¢ inventory.md â€” [summary text]
  ...
```
...
### Phase 1b â€” Registry Pre-Load (fast baseline)
...
names first â€” this is faster than reading all GDDs in full before knowing
what to look for.

If the registry is empty or absent: proceed without it. Note in the report:
"Entity registry is empty â€” consistency checks rely on full GDD reads only.
Run `/consistency-check` after this review to populate the registry."

### Phase 1c â€” L1/L2: Full Document Load
...
Phase 2 (Consistency) and Phase 3 (Design Theory) are independent â€” they read
...
```
âš ï¸  Dependency Asymmetry
[system-a].md lists: Depends On â†’ [system-b].md
[system-b].md does NOT list [system-a].md as a dependent
â†’ One of these documents has a stale dependency section
```
...
â†’ These rules directly contradict. Which GDD is authoritative?
```
...
```
âš ï¸  Stale Reference
inventory.md (written first): "Item weight uses the encumbrance formula
  from movement.md"
movement.md (written later): Defines no encumbrance formula â€” uses a flat
  carry limit instead
â†’ inventory.md references a formula that doesn't exist
```
...
```
âš ï¸  Ownership Conflict
[system-a].md Tuning Knobs: "[multiplier_name] â€” controls [output] scaling"
[system-b].md Tuning Knobs: "[multiplier_name] â€” scales [output] with [factor]"
â†’ Two GDDs define multipliers on the same output. Which owns the final value?
...
- If [system-a].md outputs values between [min]â€“[max], and [system-b].md is
  designed to receive values between [min2]â€“[max2], is the mismatch intentional?
...
```
âš ï¸  Competing Progression Loops
...
â†’ Three systems all claim to be the primary progression loop and all award
...
```
âš ï¸  Cognitive Load Risk
Simultaneously active systems during [core loop moment]:
  1. [system-a].md â€” [decision type] (active)
  2. [system-b].md â€” [resource management] (active)
  3. [system-c].md â€” [tracking] (active)
  4. [system-d].md â€” [item/action use] (active)
  5. [system-e].md â€” [cooldown/timer management] (active)
  6. [system-f].md â€” [coordination decisions] (active)
â†’ 6 simultaneously active systems during the core loop.
...
A dominant strategy makes other strategies irrelevant â€” players discover it,
...
```
âš ï¸  Potential Dominant Strategy
...
â†’ Unless melee has a significant compensating advantage (AOE, stagger,
  resource regeneration), ranged is dominant â€” higher safety, only 20% less
...
| **Positive feedback loop** | More resource â†’ easier to earn more | Runaway leader, snowball |
| **No catch-up** | Falling behind accelerates deficit | Unrecoverable states |

```
ðŸ”´ Economic Imbalance: Unbounded Positive Feedback
gold economy:
...
â†’ After equipment and abilities are purchased, gold has no sink.
...
```
âš ï¸  Difficulty Curve Mismatch
combat.md: Enemy health scales exponentially with area (Ã—2 per area)
progression.md: Player damage scales linearly with level (+10% per level)
â†’ By area 5, enemies have 32Ã— base health; player deals ~1.5Ã— base damage.
...
serves no pillar is "scope creep by design" â€” it's in the game but not in
...
```
âš ï¸  Pillar Drift
fishing-system.md: Player Fantasy â€” "peaceful, meditative activity"
Pillars: "Brutal Combat", "Tense Survival", "Emergent Stories"
â†’ The fishing system serves none of the three pillars. Either add a pillar
  that covers it, redesign it to serve an existing pillar, or cut it.
```

Also check anti-pillars â€” flag any system that does what an anti-pillar
explicitly says the game will NOT do:

```
ðŸ”´ Anti-Pillar Violation
Anti-Pillar: "We will NOT have linear story progression â€” player defines their path"
main-quest.md: Defines a 12-chapter linear story with mandatory sequence
â†’ This system directly violates the defined anti-pillar.
```

### 3g: Player Fantasy Coherence

The player fantasies across all systems should be compatible â€” they should
...
```
âš ï¸  Player Fantasy Conflict
combat.md: "You are a ruthless, precise warrior â€” every kill is earned"
dialogue.md: "You are a charismatic diplomat â€” violence is always avoidable"
exploration.md: "You are a reckless adventurer â€” diving in without a plan"
â†’ Three systems present incompatible identities. Players will feel the game
...
appear at the interaction boundary between multiple systems â€” things static
analysis of individual GDDs cannot surface.

### 4a: Identify Key Multi-System Moments

Scan all GDDs and identify the 3â€“5 most important player-facing moments where
...
1. **Trigger** â€” what player action or game event starts this?
2. **Activation order** â€” which systems activate, in what sequence?
3. **Data flow** â€” what does each system output, and is that output a valid
   input for the next system in the chain?
4. **Player experience** â€” what does the player see, hear, or feel at each step?
5. **Failure modes** â€” are there any of the following?
...
â†’ combat.md: awards kill XP (100 pts)
â†’ progression.md: XP total crosses level threshold â†’ triggers level-up
  Output: new level, stat increases, ability unlock popup
â†’ quest.md: kill-count criterion met â†’ triggers quest completion event
  Output: quest reward XP (500 pts), completion fanfare
â†’ progression.md (again): quest XP added â†’ triggers SECOND level-up in same frame
  âš ï¸  Data flow issue: quest.md awards XP without checking if a level-up
...
- **BLOCKER**: undefined behavior, broken state transition, or contradictory
  player messaging â€” the experience is broken or incoherent in this scenario
- **WARNING**: compounding spikes, feedback loops without caps, reward conflicts â€”
  the experience works but produces unintended outcomes
- **INFO**: minor ordering ambiguity or messaging overlap â€” worth noting but
...
### If FAIL â€” required actions before re-running:
...
- Options: `[A] Yes â€” update systems index` / `[B] No â€” leave as-is`
- If yes: update each flagged GDD's Status field in systems-index.md to "Needs Revision".
  (Do NOT append parentheticals to the status value â€” other skills match "Needs Revision"
...
append to `production/session-state/active.md`:
    ...
    ## Session Extract â€” /review-all-gdds [date]
    - Verdict: [PASS / CONCERNS / FAIL]
    - GDDs reviewed: [N]
...
- Are there any Warning-level items that are simple edits (flagged with "30-second edit", "brief addition", or similar)? â†’ offer inline quick-fix option
- Are any GDDs in the "Flagged for Revision" table? â†’ offer /design-review option for each
- Read systems-index.md for the next system with Status: Not Started â†’ offer /design-system option
- Is the verdict PASS or CONCERNS? â†’ offer /gate-check or /create-architecture

Build the option list dynamically â€” only include options that apply:

**Option pool:**
- `[_] Apply quick fix: [W-XX description] in [gdd-name].md â€” [effort estimate]` (one option per simple-edit warning; only for Warning-level, not Blocking)
- `[_] Run /design-review [flagged-gdd-path] â€” address flagged warnings` (one per flagged GDD, if any)
- `[_] Run /design-system [next-system] â€” next in design order` (always include, name the actual system)
- `[_] Run /create-architecture â€” begin architecture (verdict is PASS/CONCERNS)` (include if verdict is not FAIL)
- `[_] Run /gate-check â€” validate Systems Design phase gate` (include if verdict is PASS)
- `[_] Stop here`
...
1. **Surface immediately**: Report "[AgentName]: BLOCKED â€” [reason]" before continuing
...
4. **Always produce a partial report** â€” output whatever was completed so work is not lost

---

## Collaborative Protocol

1. **Read silently** â€” load all GDDs before presenting anything
2. **Show everything** â€” present the full consistency and design theory analysis
   before asking for any action
3. **Distinguish blocking from advisory** â€” not every issue needs to block
   architecture; be clear about which do
4. **Don't make design decisions** â€” flag contradictions and options, but never
   unilaterally decide which GDD is "right"
5. **Ask before writing** â€” confirm before writing the report or updating the
   systems index
6. **Be specific** â€” every issue must cite the exact GDD, section, and text
