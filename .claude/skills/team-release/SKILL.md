...
2. If a version is found: report "No version argument provided â€” inferred [version] from milestone data. Proceeding." Then confirm with `AskUserQuestion`: "Releasing [version]. Is this correct?"
...
## Team Composition
- **release-manager** â€” Release branch, versioning, changelog, deployment
- **qa-lead** â€” Test sign-off, regression suite, release quality gate
- **devops-engineer** â€” Build pipeline, artifacts, deployment automation
- **security-engineer** â€” Pre-release security audit (invoke if game has online/multiplayer features or player data)
- **analytics-engineer** â€” Verify telemetry events fire correctly and dashboards are live
- **community-manager** â€” Patch notes, launch announcement, player-facing messaging
- **producer** â€” Go/no-go decision, stakeholder communication, scheduling

## How to Delegate

Use the Task tool to spawn each team member as a subagent:
- `subagent_type: release-manager` â€” Release branch, versioning, changelog, deployment
- `subagent_type: qa-lead` â€” Test sign-off, regression suite, release quality gate
- `subagent_type: devops-engineer` â€” Build pipeline, artifacts, deployment automation
- `subagent_type: security-engineer` â€” Security audit for online/multiplayer/data features
- `subagent_type: analytics-engineer` â€” Telemetry event verification and dashboard readiness
- `subagent_type: community-manager` â€” Patch notes and launch communication
- `subagent_type: producer` â€” Go/no-go decision, stakeholder communication
- `subagent_type: network-programmer` â€” Netcode stability sign-off (invoke if game has multiplayer)
...
- Freeze the branch â€” no feature changes, bug fixes only
...
- Evaluate any open issues â€” are they blocking or can they ship?
- Make the go/no-go call
- Output: release decision with rationale

**If producer declares NO-GO:**
- Surface the decision immediately: "PRODUCER: NO-GO â€” [rationale, e.g., S1 bug found in Phase 3]."
...
- **Skip Phase 6 entirely** â€” do not tag, deploy to staging, deploy to production, or spawn community-manager.
- Produce a partial report summarizing Phases 1â€“5 and what was skipped (Phase 6) and why.
- Verdict: **BLOCKED** â€” release not deployed.
...
Verdict: **COMPLETE** â€” release executed and deployed.
Verdict: **BLOCKED** â€” release halted; go/no-go was NO or a hard blocker is unresolved.
