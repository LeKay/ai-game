...
> Usage: `/team-narrative [narrative content description]` â€” describe the story content, scene, or narrative area to work on (e.g., `boss encounter cutscene`, `faction intro dialogue`, `tutorial narrative`). Do not use `AskUserQuestion` here; output the guidance directly.
...
## Team Composition
- **narrative-director** â€” Story arcs, character design, dialogue strategy, narrative vision
- **writer** â€” Dialogue writing, lore entries, item descriptions, in-game text
- **world-builder** â€” World rules, faction design, history, geography, environmental storytelling
- **art-director** â€” Character visual design, environmental visual storytelling, cutscene/cinematic tone
- **level-designer** â€” Level layouts that serve the narrative, pacing, environmental storytelling beats

## How to Delegate

Use the Task tool to spawn each team member as a subagent:
- `subagent_type: narrative-director` â€” Story arcs, character design, narrative vision
- `subagent_type: writer` â€” Dialogue writing, lore entries, in-game text
- `subagent_type: world-builder` â€” World rules, faction design, history, geography
- `subagent_type: art-director` â€” Character visual profiles, environmental visual storytelling, cinematic tone
- `subagent_type: level-designer` â€” Level layouts that serve the narrative, pacing
- `subagent_type: localization-lead` â€” i18n validation, string key compliance, translation headroom
...
Delegate in parallel â€” issue all three Task calls simultaneously before waiting for any result:
...
- **writer**: Final self-review â€” verify no line exceeds dialogue box constraints, all text uses string keys (not raw strings), placeholder variable names are consistent
- **localization-lead**: Validate i18n compliance â€” check string key naming conventions, flag any strings with hardcoded formatting that won't survive translation, verify character limit headroom for languages that expand (German/Finnish typically +30%), confirm no cultural assumptions in text that would need locale-specific variants
...
Verdict: **COMPLETE** â€” narrative content delivered.
