...
- Do you prefer games that challenge you, relax you, tell you stories,
  or let you express yourself? *(Use `AskUserQuestion` for this â€” constrained choice.)*
...
Ask these as `AskUserQuestion` calls â€” derive the options from the chosen concept, don't hardcode them:
...
- **`creative-director`** â€” gate **CD-PILLARS** (`.claude/docs/director-gates.md`)
  Pass: full pillar set with design tests, anti-pillars, core fantasy, unique hook.

- **`art-director`** â€” gate **AD-CONCEPT-VISUAL** (`.claude/docs/director-gates.md`)
...
- **Target platform**: Use `AskUserQuestion` â€” "What platforms are you targeting for this game?"
...
- **Engine experience**: Use `AskUserQuestion` â€” "Do you already have an engine you work in?"
...
If [B]: ask which section to revise using `AskUserQuestion` with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else â€” I'll describe`

After revising, show the updated section as a diff or clear before/after, then use `AskUserQuestion` â€” "Ready to write the updated concept document?"
