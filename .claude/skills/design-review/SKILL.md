...
**Dependency graph validation:** For every system listed in the Dependencies section, use Glob to check whether its GDD file exists in `design/gdd/`. Flag any that don't exist yet â€” these are broken references that downstream authors will hit.
...
**Prior review check:** Check whether `design/gdd/reviews/[doc-name]-review-log.md` exists. If it does, read the most recent entry â€” note what verdict was given and what blocking items were listed. This session is a re-review; track whether prior items were addressed.
...
> "Full review: spawning specialist agents in parallel. This typically takes 8â€“15 minutes. Use `--review lean` for faster single-session analysis."
...
Work through all blocking items, asking for design decisions only where you cannot resolve the issue from the GDD and existing docs alone. Group all design-decision questions into a single multi-tab `AskUserQuestion` before making any edits â€” do not interrupt mid-revision for each blocker individually.

After all revisions are complete, show a summary table (blocker â†’ fix applied) and use `AskUserQuestion` for a **post-revision closing widget**:
...
- `design/gdd/systems-index.md` â€” find any system with Status: In Review or NEEDS REVISION (other than the one just reviewed)
- Count `.md` files in `design/gdd/` (excluding game-concept.md, systems-index.md) to determine if `/review-all-gdds` is worth offering (â‰¥2 GDDs)
...
- `[_] Run /consistency-check â€” verify this GDD's values don't conflict with existing GDDs` (always include if â‰¥1 other GDD exists)
- `[_] Run /review-all-gdds â€” holistic design-theory review across all designed systems` (include if â‰¥2 GDDs exist)
