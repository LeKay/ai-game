...
**ADR Decision Summary**: Three `TileMapLayer` nodes (TerrainLayer, ResourceOverlay, BuildingSlots) are children of `MapRoot (Node2D)`. Each has its own `TileSet` with `tile_size = Vector2i(48, 48)`. After `GridMap.generate()`, a batch `set_cell()` pass populates all TileMapLayer cells. Data flows one way: Grid → TileMapLayer. `TileMap` (deprecated since 4.3) must not appear anywhere.
...
- [ ] **Rendering AC-A**: `MapRoot.tscn` scene contains exactly 3 `TileMapLayer` children (`TerrainLayer`, `ResourceOverlay`, `BuildingSlots`) and zero `TileMap` nodes
- [ ] **Rendering AC-B**: After `GridMap.generate(42)`, every tile position in the 30×30 grid has a corresponding `set_cell()` call on `TerrainLayer` matching its `TileType`
...
├── TerrainLayer (TileMapLayer)    — terrain tiles, TileSet: terrain_tileset.tres
├── ResourceOverlay (TileMapLayer) — resource indicator tiles, TileSet: resource_tileset.tres
├── BuildingSlots (TileMapLayer)   — 1×1 slot indicators (optional visual), TileSet: slot_tileset.tres
└── Grid (GridMap)                 — pure data node, no rendering
```
...
**Batch set_cell after generation**: Add `_sync_tilemap()` method to `MapRoot` (or `GridMap`), called once after `generate()`:
```gdscript
func _sync_tilemap() -> void:
    for x in range(GridMap.GRID_SIZE):
        for y in range(GridMap.GRID_SIZE):
...
```
This call happens in `MapRoot` (which owns both GridMap and TileMapLayer nodes), not inside `GridMap` itself. GridMap updates data; MapRoot syncs rendering.
...
- Story 001: GridMap data model (reads grid data via `get_terrain`, `get_resource`)
- Story 002: `generate(seed)` — generation must be done before rendering can sync
- Story 003: `place_building` — placement logic lives in GridMap; rendering reaction is wired here
...
- **Rendering AC-A**: No TileMap nodes in scene
  ...
  - Verify: Scene tree shows 3 `TileMapLayer` children; no `TileMap` node anywhere
  - Pass condition: Zero `TileMap` nodes; three `TileMapLayer` nodes present
...
- Depends on: Story 001 must be DONE (GridMap class with read API), Story 002 must be DONE (generation fills arrays that rendering syncs from)
